...
const POSTHOG_STORAGE_CHECK = `function rlrLocalStorageAvailable(){try{if(!window.localStorage)return!1;var k="__rlr_storage_test__";window.localStorage.setItem(k,"1");window.localStorage.removeItem(k);return!0}catch(e){return!1}}`;

export function PostHogHeadScript() {
  const apiKey = process.env.NEXT_PUBLIC_POSTHOG_KEY?.trim();
  const apiHost = process.env.NEXT_PUBLIC_POSTHOG_HOST ?? "https://d.paevo.co";
  ...
  return (
    <Script
      ...
      dangerouslySetInnerHTML={{
        __html: `${POSTHOG_LOADER}
${POSTHOG_STORAGE_CHECK}
(function(){var cfg=${initConfig};cfg.persistence=rlrLocalStorageAvailable()?"localStorage+cookie":"cookie";posthog.init(${JSON.stringify(apiKey)},cfg);})();`,
      }}
    />
  );
}
