#!/usr/bin/env python3
"""Export Meridian Platform verification output as a frontend demo fixture JSON."""
...
def _slug_for_finding(finding: RuleFinding, index: int) -> str:
    ...
    if finding.subscription_id:
        sub_suffix = str(finding.subscription_id).replace("-", "")[:8]
        return f"{base}-{sub_suffix}"
    return f"{base}-{index}"
...
def build_fixture() -> dict:
    ...
    accounts = len(ctx.customers)
    invoices = len(ctx.invoices)
    recoverable = Decimal(reconciliation["headline_recoverable_arr"])
    leakage_pct = (recoverable / Decimal(str(ARR_TARGET_USD)) * 100).quantize(Decimal("0.1"))

    narrative = (
        f"{COMPANY_NAME}'s billing exports reveal {reconciliation['primary_findings']} primary revenue "
        f"leakage findings totaling {_format_currency(recoverable)} in recoverable ARR against a "
        f"${ARR_TARGET_USD // 1_000_000}M subscription base ({leakage_pct}% of run-rate ARR). "
        "The largest concentrations are expired promotional discounts still billing after the EOY25 "
        "campaign, legacy catalog pricing after the July 2024 price increase, and invoice execution "
        "drift on renewals. Each finding includes subscription-level evidence and a recommended "
        "remediation path."
    )
